...
def build_scaffolds_from_scn_angles(seq, angles=None, coords=None, device="cpu"):
    ...
    cloud_mask = scn_cloud_mask(seq, coords=coords).bool().to(device)
    ...
    return cloud_mask, point_ref_mask, angles_mask, bond_mask
...
def modify_scaffolds_with_coords(scaffolds, coords):
    ...
    for i in range(3, 14):
        ...
        # get angles
        scaffolds["angles_mask"][0, :, i] = compute_angle(coords[selector, idx_b],
        ...
        # get dihedrals
        scaffolds["angles_mask"][1, :, i] = compute_dihedral(coords_a,
                                                             coords[selector, idx_b],
                                                             coords[selector, idx_c],
                                                             coords[:, i])
    # correct angles and dihedrals for backbone 
    scaffolds["angles_mask"][0, :-1, 0] = compute_angle(coords[:-1, 1], coords[:-1, 2], coords[1:, 0])  #  ca_c_n
    scaffolds["angles_mask"][0, 1:, 1] = compute_angle(coords[:-1, 2], coords[1:, 0], coords[1:, 1])  #  c_n_ca
    scaffolds["angles_mask"][0, :, 2] = compute_angle(coords[:, 0], coords[:, 1], coords[:, 2])  # n_ca_c

    # N determined by previous psi = f(n, ca, c, n+1)
    scaffolds["angles_mask"][1, :-1, 0] = compute_dihedral(coords[:-1, 0], coords[:-1, 1], coords[:-1, 2],
                                                           coords[1:, 0])
    # CA determined by omega = f(ca, c, n+1, ca+1)
    scaffolds["angles_mask"][1, 1:, 1] = compute_dihedral(coords[:-1, 1], coords[:-1, 2], coords[1:, 0], coords[1:, 1])
    # C determined by phi = f(c-1, n, ca, c)
    scaffolds["angles_mask"][1, 1:, 2] = compute_dihedral(coords[:-1, 2], coords[1:, 0], coords[1:, 1], coords[1:, 2])
